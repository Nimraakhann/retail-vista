...
from .models import UserProfile, Camera
...
def update_alert_evidence(request):
    """
    Update an existing alert with completed video evidence
    Used when an in-progress alert is finalized with the full video
    """
    try:
        data = request.data
        alert_id = data.get('alert_id')
        ...
        if not video_clip:
            return Response({'error': 'No video evidence provided'}, status=status.HTTP_400_BAD_REQUEST)
